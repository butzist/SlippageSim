...
def do_sim(d,h,r):
   ...
   scale_tree(tree,h)
   hmm = HMMERProfile.HMMERProfile("GALA_LRR.hmm",count=10,binary="hmmemit")
   profiles = iter(hmm)
   ...
   for i in xrange(6):
      events = [
         ...
         Bio2.SeqEvolution.GeometricDeletionFactory(rate=0.05,alen=3.5)
      ]
      mut = Bio2.SeqEvolution.mutate_sequence(repeat,0.2,events,return_all=True)
      repeats += mut.seq
      homology += mut.mapping
   ...
   events = [
               ...
               SlippageSim.SlippageDupFactory(ins_rate=.5*r,del_rate=.5*r,length_distr=scipy.stats.geom(1.0/50,0)),
